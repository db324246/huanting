<template>
  <div>
    ...
    <div class="ctr_row">
      <div class="btn_group">
        <el-button icon="el-icon-delete" size="mini" type="danger" @click="batchDisabledUsers('Y')"> 批量禁用 </el-button>
        <el-button icon="el-icon-delete" size="mini" type="primary" @click="batchDisabledUsers('N')"> 批量解禁 </el-button>
      </div>
    ...
    <el-table
      ...
      <el-table-column
        ...
        label="状态">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.disabled === 'N'" type="success">活跃</el-tag>
          <el-tag v-if="scope.row.disabled === 'Y'" type="danger">禁用</el-tag>
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    selectionChange(selection) {
      ...
      if (length > 1) {
        ...
        const disabledLength = selection.filter(item => item.disabled === disabled).length
    ...
    batchDisabledUsers(disabled) {
      if (this.ids.length === 0) return this.$message.error('请选择需要操作的用户')

      this.$confirm(`确认${disabled === 'Y' ? '禁用' : '解禁'}?`, '提示', {
        confirmButtonText: '确认',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        userDisabled({ ids: this.ids, disabled }).then(res => {
          if (res.success) {
