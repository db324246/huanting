      ...
      <el-scrollbar style="height: 100%;">
        ...
        <el-table
          ...
          <el-table-column
            ...
            label="照片">
            <template slot-scope="scope">
              <img :src="scope.row.imageSrc" alt="" class="table_img" @click="previewImg(scope)" :onerror="defaultImg">
            </template>
          </el-table-column>
        ...
        <el-dialog
          ...
          <el-form style="padding-right: 50px;" ref="singerForm" :rules="rules" :model="singerForm" label-width="80px">
            ...
            <el-form-item label="照片" prop="imageSrc">
              <image-upload v-model="singerForm.imageSrc" :on-success="handleImgSuccess">
              </image-upload>
  ...
  methods: {
    ...
    getSingerList() {
      singerTable(this.queryList).then(res => {
        ...
        this.imgPreviewConfig.imageContent = res.data.records.map(item => item.name)
      }).catch(err => {
        console.log(err);
      })
    },
